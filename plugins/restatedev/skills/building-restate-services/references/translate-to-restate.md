...
| Retry policy | Service/handler retry policy or `ctx.run()` retry options. (see https://docs.restate.dev/guides/error-handling) | Retry counter in state, try-catch retry loop |
...
| Relational database                             | Keep it + `ctx.run()` database calls: https://docs.restate.dev/guides/databases | Complex access patterns. Full SQL. Access from other services.                                        |
